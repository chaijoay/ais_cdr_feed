...
**  10.1a   03-Aug-2010     Calculate usc's charge using FixCharge, in case FixCharge is NULL use ChargeRate instead.
...
**  11.03   08-Oct-2013     Add condition Reject Call in SGSN cdr FMS support New cdr type GGSN (UR-fms-ggsn-8Oct13-FMS_08102013)
**  11.04   05-Nov-2013     Add AWN IDD and Ignore logic for USC (UR-FMS_28102013-IDD003 and UR-FMS-smsgw-05112013)
**  11.05   13-Nov-2013     Add IMEI lookup table for BOS subscriber (FMS-IMEI-11112013)
**  11.06   21-Feb-2014     re-format Error and Ignore message in order to easily categorize
**  11.07   04-Mar-2014     sort event output before pass to IUM Processes
**  11.08   09-May-2014     remove CAT_TOTAL out of voice local for CAT_ONNET and CAT_OFFNET
**  11.09   20-May-2014     new added VSCP(GSMCT, ctudr) CDR for Google Play
**  11.10   28-Sep-2015     new added IMS type (4g VoWIFI and VoLTE)
...
int main(int argc, char **argv)
{
    ...
    if ( Init_Main(argv[0]) ) {
        writeLog(LOG_INF, "------------------ exit %s ------------------", APP_NAME);
        Close_Main();
        return FAILURE;
    }
    ...
    switch ( Anz_Check_File() ) {
        ...
        case ANZ_MAP_HALF :
            ...
            if ( cGblShutdown ) {
                ...
                writeLog(LOG_INF, "------------------ terminate %s ------------------", APP_NAME);
                Close_Main();
                return SUCCESS;
            }
            break;
...
int Init_Main (const char *avg0)
{
    ...
    *p = '\0';
    ...
    sprintf(gszIniFullName, "%s/%s%s", appPath, APP_NAME, INI_EXT);
    ...
    if ( glb_DbgFlag ) {
        nResult = startLogging(Log_Dir, APP_NAME, LOG_DBG);
    }
    else {
        nResult = startLogging(Log_Dir, APP_NAME, LOG_INF);
    }
    ...
    writeLog(LOG_INF, "%s v.%s, for %s", APP_NAME, APP_VERSION, APP_PURPOSE);
